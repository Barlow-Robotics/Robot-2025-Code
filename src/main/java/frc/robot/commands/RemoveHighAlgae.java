...
public class RemoveHighAlgae {
    ...
    public RemoveHighAlgae(ArmStateManager asm, Elevator e, Arm a, Wrist w, Gripper g) {
        armStateManager = asm;
        theElevator = e;
        theArm = a;
        theWrist = w;
    ...
    public Command command() {
        return Commands.defer(
                () -> {
                    ...
                    if (currentState != ArmState.StartAlgaeHighPosition) {
                        // we need to position the gripper for the next button press
                        return Commands.sequence(
                                new PositionGripper(armStateManager, ArmState.StartAlgaeHighPosition, theElevator, theArm,
                                        theWrist)
                                        .command());
                    } else {
                        return Commands.sequence(
